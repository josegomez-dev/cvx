...
export function HackathonCard({ achievement, index }: HackathonCardProps) {
  ...
  return (
    ...
    >
              {/* Category Badge */}
        <motion.div 
          className={`absolute top-4 right-4 inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-full bg-gradient-to-r ${getCategoryColor(achievement.category)} text-xs font-medium cursor-pointer`}
          whileHover={{ 
            scale: 1.1,
            y: -2,
            rotateZ: 2,
